  ...
  const renderRow = (item: EventList) => (
    ...
    >
      <td className="font-semibold pl-2">
        <Link href={`/list/events/${item.entityId}`}>
          {item.name}
        </Link>
      </td>
      <td className="hidden md:table-cell">{getPersonName(item.clientPersonEntityId)}</td>
        ...
        <div className="flex items-center gap-2">
          ...
          {role === "admin" && (
            <>
              ...
              <FormContainer table="events" type="delete" id={item.entityId} />
            </>
          )}
        </div>
      </td>
